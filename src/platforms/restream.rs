...
fn get_restream_config(ctx: &RouteContext<()>, legacy: bool) -> Result<OAuthConfig> {
    let config = oauth::OAuthConfig {
        ...
        redirect_url: match legacy {
            false => ctx.var("RESTREAM_REDIRECT_URL")?.to_string(),
            true => ctx.var("RESTREAM_LEGACY_REDIRECT_URL")?.to_string(),
        },
        scope: SCOPES.to_string(),
        auth_url: RESTREAM_AUTH_URL.to_string(),
        token_url: RESTREAM_TOKEN_URL.to_string(),
        extra_params: HashMap::from([("include_granted_scopes".to_string(), "true".to_string())]),
    };
